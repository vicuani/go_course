...
func main() {
	...
	err = processContacts(inputFile, outputFile)
	if err != nil {
		panic(err)
	}
...
func processContacts(input io.Reader, output io.Writer) error {
	...
	*/
	...
	for scanner.Scan() {
		...
		if len(name) > 1 {
			...
			cleanPhone = extractDigits(digitsPattern, cleanPhone)
			...
			_, err = output.Write([]byte(outputLine))
			if err != nil {
				return err
			}
		}
...
func extractDigits(digitsPattern *regexp.Regexp, phone string) string {
	digits := digitsPattern.FindAllString(phone, -1)
...
func formatPhone(digits string) (string, error) {
	switch len(digits) {
	case 12:
		return fmt.Sprintf("(%s) %s-%s-%s", digits[2:5], digits[5:7], digits[7:9], digits[9:]), nil // For +380 (12 digits)
	case 10:
